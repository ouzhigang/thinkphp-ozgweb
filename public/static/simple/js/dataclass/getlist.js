...
function show_data(data) {
	...
	if(data) {
		for(var i = 0; i < data.length; i++) {
			...
			item += "<div>";
			item += "<button type=\"button\" id=\"btn_edit_" + data[i].id + "\" class=\"btn btn-link\" req-data=\"add?type=" + data[i].type + "&id=" + data[i].id + "\">编辑</button>";
			item += "<button type=\"button\" id=\"btn_del_" + data[i].id + "\" class=\"btn btn-link\" req-data=\"del?id=" + data[i].id + "\">删除</button>";
			item += "</div>";
			item += "</li>";
		...
		success: function(res, status) {
			...
			if(res.code == 0) {
				...
				for(var i = 0; i < res.data.length; i++) {
					...
					item += "<div>";
					item += "<button type=\"button\" id=\"btn_edit_" + res.data[i].id + "\" class=\"btn btn-link\" req-data=\"add?type=" + res.data[i].type + "&id=" + res.data[i].id + "\">编辑</button>";
					item += "<button type=\"button\" id=\"btn_del_" + res.data[i].id + "\" class=\"btn btn-link\" req-data=\"del?id=" + res.data[i].id + "\">删除</button>";
					item += "</div>";
					item += "</li>";
