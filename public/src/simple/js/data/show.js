...
function show_data(page) {
	...
	$.ajax({
		url: cfg.web_root + "simple/data/show",
		type: "get",
		dataType: "json",
		data: data,
		beforeSend: function() {
		...
		success: function(res, status) {
			if(res.code == 0) {
				...
				for(var i = 0; i < res.data.list.length; i++) {				
					...
					row += "<td>";
					row += "<button class=\"btn btn-outline btn-link btn-xs\" type=\"button\" id=\"btn_edit_" + res.data.list[i].id + "\" data-id=\"" + res.data.list[i].id + "\">编辑</button>";
				...
				$("button[id ^= 'btn_edit_']").click(function() {
					...
					$.get(cfg.web_root + "simple/data/get", data, function(res) {
						if(res.code == 0) {
