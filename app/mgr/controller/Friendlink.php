...
class Friendlink extends \app\BaseController {
	...
	public function show() {
		...
		$page_size = input("param.page_size/d", config("app.web_page_size"));
		...
		//分页导航
		$page_first = config("app.web_root") . "mgr/friendlink/show?page=1";
		$page_prev = config("app.web_root") . "mgr/friendlink/show?page=" . ($page <= 1 ? 1 : $page - 1);
		$page_next = config("app.web_root") . "mgr/friendlink/show?page=" . ($page >= $data["page_count"] ? $data["page_count"] : $page + 1);
		$page_last = config("app.web_root") . "mgr/friendlink/show?page=" . $data["page_count"];
